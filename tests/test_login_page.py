...
# Method to validate a invalid login
...
# Method to validate the process forgot paswword
def test_forgot_password(web_drivers):
    expected_msg = "An email with a confirmation link has been sent your email address."
    login_page = LoginPage(*web_drivers)
    login_page.open()
    login_page.forgot_password("tester@qaminds.com")
    actual_msg = login_page.get_success_msg_forgot_p()
    assert expected_msg == actual_msg, f"Success mesagge should {expected_msg}"


# Method to validate a valid login
